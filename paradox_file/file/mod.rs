...
impl LocalFile {
    pub(crate) fn get_filename<T: AsFilename>(
        ...
        Ok(Filename(Self::get_name_inner(
            &mut PathBuf::from(base_path),
            sub_directory,
            filename,
        )?))
    }
    ...
    pub(crate) fn convert_name(
        &mut self,
        convert_name: impl Fn(Option<String>) -> String,
    ) -> io::Result<()> {
        ...
        let filename = self
            .path
            .file_name()
            .ok_or(ErrorKind::InvalidData.into_error("Filename could not be read"))?
            .to_str()
