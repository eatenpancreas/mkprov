...
use crate::{LexerError, Object, ParserError};
...
impl PdxFile {
    ...
    pub fn rename_prov_name(&mut self, id: u16, to: String) -> io::Result<()> {
        self.file.convert_name(|_| format!("{} - {}.txt", id, to))
    }
    ...
    pub fn inspect<T: AsFilename>(
        ...
        if let Ok(mod_file) =
            LocalFile::get_file(cfg.require_mod_directory()?, sub_directory, filename)
        {
            file_contents = mod_file.get_contents()?;
        } else if let Ok(game_file) =
            LocalFile::get_file(cfg.require_game_directory()?, sub_directory, filename)
        {
    ...
    pub fn pull<T: AsFilename>(
            ...
            LocalFile::get_file(cfg.require_mod_directory()?, sub_directory, filename)
        {
            ...
            LocalFile::get_filename(cfg.require_game_directory()?, sub_directory, filename)
        {
            let game_file =
                LocalFile::get_file(cfg.require_game_directory()?, sub_directory, &filename)?;
            file_contents = game_file.get_contents()?;
